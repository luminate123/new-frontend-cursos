...
import { Clock3, Loader2, Receipt, TrendingUp, Users, Wallet } from 'lucide-react';
import { toast } from 'sonner';
import { Button } from '@/components/ui/button';
import { StatTile } from '@/components/ui/stat-tile';
import { formatPEN, LINE_LABELS } from '@/lib/api/courses';
import type { Revenue } from '@/lib/api/payments';

/**
 * Vista de ingresos. La comparten el administrador (toda la academia) y el
 * instructor (solo sus programas): cambia quién la alimenta, no cómo se lee.
 *
 * ponytail: un componente con un `fetcher`, en vez de dos páginas gemelas que
 * se desincronizan a la tercera semana.
 */
...
export function RevenueView({
  ...
  subtitle,
  fetcher,
  courseColumnLabel = 'Programa',
  emptyCourses = 'Todavía no hay ventas registradas.',
  actions,
}: {
  title: string;
  subtitle: string;
  fetcher: (range: { from?: string; to?: string }) => Promise<Revenue>;
  ...
  const load = useCallback(async () => {
    setLoading(true);
    try {
      setData(await fetcher({ from: from || undefined, to: to || undefined }));
    } catch {
  ...
  return (
    <div>
      ...
      {loading ? (
        ...
        <p className="mt-10 text-center text-sm text-muted-foreground">No hay datos para mostrar.</p>
      ) : (
        <>
          ...
          {/* Solo la vista general del admin trae desglose por instructor. */}
          {byInstructor.length > 0 && (
            <section className="mt-6 rounded-2xl border border-border bg-card p-6 shadow-card">
              <h2 className="flex items-center gap-2 text-sm font-semibold">
                <Users className="h-4 w-4 text-muted-foreground" />
                Ingreso por instructor
              </h2>
                ...
                <table className="w-full min-w-[480px] text-sm">
                  <thead>
                    <tr className="border-b border-border text-left text-xs text-muted-foreground">
                      ...
                      <th className="py-2 text-right font-medium">Programas</th>
                  ...
                  <tbody>
                    {byInstructor.map((row) => (
                      <tr key={row.instructorId} className="border-b border-border/60">
                        <td className="py-2.5 font-medium">{row.name}</td>
                        <td className="py-2.5 text-right tabular-nums text-muted-foreground">
                          {row.courses}
                        </td>
                        <td className="py-2.5 text-right tabular-nums">{row.sales}</td>
          ...
          <section className="mt-6 rounded-2xl border border-border bg-card p-6 shadow-card">
            <h2 className="text-sm font-semibold">Ingreso por {courseColumnLabel.toLowerCase()}</h2>

            {data.byCourse.length === 0 ? (
              <p className="py-10 text-center text-sm text-muted-foreground">{emptyCourses}</p>
            ) : (
              <div className="mt-4 overflow-x-auto">
                <table className="w-full min-w-[520px] text-sm">
                  <thead>
                    <tr className="border-b border-border text-left text-xs text-muted-foreground">
                      <th className="py-2 font-medium">{courseColumnLabel}</th>
